"""
Evaluation for Task 2b: Contrastive pre-training experiments.

Entry point: evaluate(config) — called by run.py, not directly.

Loads the best finetune checkpoint, runs TTA (8 views) on the test set,
and saves metrics (JSON + TXT) and a confusion matrix PNG.
"""
...
from model   import NucleiResNet, ResNet18Encoder
from dataset import TestDataset, CLASS_NAMES, NUM_CLASSES   # noqa: F811 (from 2a via sys.path)
...
def _load_model(checkpoint_path: str) -> nn.Module:
    ...
    cfg      = ckpt.get("config", {})
    backbone = cfg.get("backbone", ckpt.get("backbone", "nuclresnet"))
    use_eca  = cfg.get("use_eca", True)
    fdim     = cfg.get("fdim", None)
...
def _print_results(tag: str, metrics: dict, src_bkdn: dict) -> None:
    print(f"\n{'='*60}")
    print(f"  Results: {tag}")
    print(f"{'='*60}")
...
def evaluate(config: dict) -> None:
    """
    Evaluate the best finetune checkpoint on the test set with TTA.
    """
    ...
    print(f"  Backbone     : {ckpt.get('backbone', '?')}")
    ...
    metrics  = _compute_metrics(result["preds"], result["targets"])
    src_bkdn = _source_breakdown(result["preds"], result["targets"], result["sources"])

    _print_results(exp_name, metrics, src_bkdn)
    print()
    print(classification_report(result["targets"], result["preds"],
                                 target_names=CLASS_NAMES, digits=4, zero_division=0))

    # ---- Confusion matrix ----
    cm_path = str(out_dir / "confusion_matrix_test.png")
    _save_confusion_matrix(result["preds"], result["targets"], cm_path,
    ...
    with open(txt_path, "w") as f:
        ...
        f.write(f"Backbone         : {ckpt.get('backbone', '?')}\n")
        f.write(f"Saved epoch      : {saved_epoch}\n")
        f.write(f"Saved val acc    : {saved_acc}\n")
        f.write(f"Test samples     : {len(result['targets'])}\n\n")
        ...
        f.write("\n")
        f.write(classification_report(result["targets"], result["preds"],
                                       target_names=CLASS_NAMES, digits=4, zero_division=0))
