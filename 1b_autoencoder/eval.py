...
def _build_model(config: dict) -> torch.nn.Module:
    return Autoencoder(mode="finetune", base=64,
...
def _compute_metrics(model: torch.nn.Module, loader: DataLoader) -> dict:
    """Per-image Dice and IoU, skipping absent classes."""
...
def _qualitative(model: torch.nn.Module, dataset: TissueDataset,
                 save_dir: Path, label: str) -> None:
...
def evaluate(config: dict) -> None:
    """Evaluate both best and last checkpoints for the given experiment."""
    name     = config["name"]
    ...
    for label in ("best", "last"):
        ckpt = run_dir / f"ae_finetune_{label}.pt"
        if not ckpt.exists():
